...
(async function () {
   ...
   console.log(depositaddress)
    ...
    app.post("/", async function (req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (req.body.txid) {
                ...
                obj.vout.forEach((singlevout) => {
                    let searchResult = singlevout.scriptPubKey.addresses?.find((address) => address === depositaddress)
                ...
                if (obj.vin) {
                    ...
                    if (txid) {
                        ...
                        let outputSearchResult = sendertx.vout.find((single) => single.n === vout)
                        let sender = outputSearchResult?.scriptPubKey.addresses?.pop()
                        console.log("Amount:",value)
                        console.log("TXID: ",req.body.txid);
                        console.log("Timestamp:",Date.now())
                        if(!investments.find((investment) => investment.txid === req.body.txid)) {
                            investments.push({address: sender!,amount: value,txid:req.body.txid,
                                timestamp: Date.now(),paid: false})
                                db.run("INSERT INTO investments(address,amount,txid,timestamp,paid) VALUES(?,?,?,?,?)",{address: sender!,amount: value,txid:req.body.txid,
                                    timestamp: Date.now(),paid: false},(result: any,error: any) => {
                                        console.log(result)
                                        console.log(error)
                                    })
                        }
